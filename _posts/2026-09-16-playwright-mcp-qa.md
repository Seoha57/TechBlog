...
Playwright MCP는 탐색과 초안 작성에는 유용하지만, 테스트 결과를 그대로 신뢰하면 안 됩니다. 같은 요청이라도 페이지 상태나 locator 선택에 따라 결과가 달라질 수 있고, 로그인 정보나 개인정보가 포함된 환경에서는 데이터 노출 위험도 있습니다.

실무에서는 다음 기준으로 범위를 나누는 것이 안전합니다.

- MCP: 신규 기능 탐색, 테스트 시나리오 초안, 재현 절차 확인
- Playwright 코드: 반복 회귀 테스트, CI 실행, 릴리스 검증
- QA 리뷰: 기대 결과, 예외 조건, 데이터 정합성 최종 확인

결국 MCP는 QA 엔지니어의 판단을 대체하는 도구가 아니라, 브라우저를 직접 확인하는 시간을 줄여주는 도구에 가깝습니다. 작은 시나리오부터 검증하고, 재현성과 안정성이 확인된 흐름만 자동화 테스트로 승격하는 방식이 현실적입니다.

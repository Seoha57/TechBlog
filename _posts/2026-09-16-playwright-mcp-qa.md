...
Playwright MCP는 QA 업무의 시작점을 바꿀 수 있습니다. 기존에는 화면을 확인하고 locator를 찾은 뒤 코드를 작성해야 했다면, MCP를 이용하면 자연어로 사용자 흐름을 먼저 탐색할 수 있습니다. 이 과정은 신규 기능의 테스트 가능성을 판단하거나, 수동 재현 절차를 자동화 코드로 옮길 때 특히 유용합니다.
...
이번 실습의 결론은 간단합니다. MCP로 빠르게 탐색하고, Playwright 코드로 반복 가능하게 만들고, QA 리뷰로 의미를 확인합니다. 세 단계를 분리하면 새로운 기술을 받아들이면서도 기존 테스트 품질 기준을 유지할 수 있습니다.
